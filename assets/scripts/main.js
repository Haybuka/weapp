...
window.addEventListener('load', () => {
	...
	function errDom(data) {
		...
		humidityPercent.setAttribute('class', 'measurement--span js--humidity-percent');
		...
		let headings = [dayOneHeader, dayTwoHeader, dayThreeHeader, dayFourHeader, dayFiveHeader];
		for (let i = 0; i < headings.length; i++) {
			headings[i].setAttribute('class', 'heading-headers');
		}
		//setting icon attribute

		dayOneimg.setAttribute('src', `https://openweathermap.org/img/wn/${data.daily[1].weather[0].icon}@2x.png`);
		dayTwoimg.setAttribute('src', `https://openweathermap.org/img/wn/${data.daily[2].weather[0].icon}@2x.png`);
		dayThreeimg.setAttribute('src', `https://openweathermap.org/img/wn/${data.daily[3].weather[0].icon}@2x.png`);
		dayFourimg.setAttribute('src', `https://openweathermap.org/img/wn/${data.daily[4].weather[0].icon}@2x.png`);
		dayFiveimg.setAttribute('src', `https://openweathermap.org/img/wn/${data.daily[5].weather[0].icon}@2x.png`);
		...
		//day one
		dayOneDegree.innerHTML = `${Math.round(data.daily[1].temp.day - 273.15)} &deg; c`;
		dayOneDay.innerHTML = `Date`;
		dayOneDate.innerHTML = `${date + 1} / ${month}`;
		//day two
		dayTwoDegree.innerHTML = `${Math.round(data.daily[2].temp.day - 273.15)} &deg; c`;
		dayTwoDay.innerHTML = `Date`;
		dayTwoDate.innerHTML = `${date + 2} / ${month}`;
		//day three
		dayThreeDegree.innerHTML = `${Math.round(data.daily[3].temp.day - 273.15)} &deg; c`;
		dayThreeDay.innerHTML = `Date`;
		dayThreeDate.innerHTML = `${date + 3} / ${month}`;
		//day four
		dayFourDegree.innerHTML = `${Math.round(data.daily[4].temp.day - 273.15)} &deg; c`;
		dayFourDay.innerHTML = `Date`;
		dayFourDate.innerHTML = `${date + 4} / ${month}`;
		//day five
		dayFiveDegree.innerHTML = `${Math.round(data.daily[5].temp.day - 273.15)} &deg; c`;
	...
	function widget() {
		...
		navWidget.addEventListener('click', function (params) {
			...
			widgetClose.addEventListener('click', () => {
				widget.classList.remove('open');
				// daySection.classList.remove('reduce');
				// currentSection.classList.remove('reduce');
				// measureAside.classList.remove('reduce');
			});
		});
	}

	function getSearch(newValue) {
		// https://api.openweathermap.org/data/2.5/weather?q=${newValue}&exclude=minutely,hourly&appid=fa060ce23674f3a0d7f0e80a98248955
